...
def create_guarded_backup(canonical: Path, backup: Path) -> sqlite3.Connection:
    """Create an exact D-029 backup, then acquire the canonical write lock.

    Windows SQLite uses mandatory byte-range locks inside the database file. Reading
    the raw file after BEGIN IMMEDIATE/EXCLUSIVE can therefore fail around SQLite's
    locking bytes. Build and verify the byte-identical backup first, while a live
    SQLite connection tracks ``PRAGMA data_version``. Then acquire BEGIN IMMEDIATE
    and re-check data_version + file metadata before allowing any canonical write.
    """
    ...
    try:
        ...
        if (
            ...
            raise RuntimeError(
                "Canonical DB changed while the D-029 backup was being created"
            )
        ...
        for suffix in ("-wal", "-shm", "-journal"):
            ...
            if sidecar.exists():
                raise RuntimeError(
                    f"Canonical DB acquired a SQLite sidecar during backup: {sidecar}"
                )
        ...
        if (
            ...
            raise RuntimeError(
                "Canonical DB changed before the promotion write lock was secured"
            )
        ...
        if migration != EXPECTED_BASELINE_MIGRATION:
            raise RuntimeError(
                "Canonical migration changed before promotion: "
...
def promote_canonical(
    ...
    backup_ready = False
    try:
        progress("[promote] creating verified D-029 backup and acquiring write lock")
