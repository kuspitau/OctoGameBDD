...
if exist "%DEFAULT_DB%" (
    call :hash_matches "%DEFAULT_DB%"
    if "%HASH_MATCH%"=="1" (
        set "CANONICAL_DB=%DEFAULT_DB%"
...
>>"%DISCOVERY_PS%" echo   }
>>"%DISCOVERY_PS%" echo }
>>"%DISCOVERY_PS%" echo $valid ^| Sort-Object -Unique ^| Set-Content -LiteralPath $OutFile -Encoding UTF8
...
if "%DB_COUNT%"=="1" (
    ...
    call :log INFO "Canonical DB: %CANONICAL_DB%"
    goto :db_found
)
...
:compute_hash
set "LAST_HASH="
for /f "usebackq delims=" %%H in (`powershell -NoProfile -Command "(Get-FileHash -LiteralPath '%~1' -Algorithm SHA256).Hash.ToLowerInvariant()"`) do set "LAST_HASH=%%H"
exit /b 0
